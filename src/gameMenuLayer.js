...
var GameMenuLayer = cc.Layer.extend({
    ...
    initMenuLayer: function(){
        ...
        key_z.setPosition(cc.p(180, baselayer.getContentSize().height*3/4));
        baselayer.addChild(key_z);

        var key_x = new cc.Sprite(res.key_X);
        key_x.setPosition(cc.p(180, baselayer.getContentSize().height*3/8));
        baselayer.addChild(key_x);

        var controllabel = new cc.LabelTTF("← Control Move And Direction →", "", 15);
        controllabel.setPosition(cc.p(baselayer.getContentSize().width/2, baselayer.getContentSize().height*3/4));
        controllabel.setColor(cc.color(100,100,100,100));
        baselayer.addChild(controllabel);

        var key_dot = new cc.Sprite(res.key_dot);
        key_dot.setPosition(cc.p(baselayer.getContentSize().width-180, baselayer.getContentSize().height*3/4));
        baselayer.addChild(key_dot);

        var key_slash = new cc.Sprite(res.key_slash);
        key_slash.setPosition(cc.p(baselayer.getContentSize().width-180, baselayer.getContentSize().height*3/8));
        baselayer.addChild(key_slash);

        var shootlabel = new cc.LabelTTF("← Click To Shoot →", "", 15);
        shootlabel.setPosition(cc.p(baselayer.getContentSize().width/2, baselayer.getContentSize().height*3/8));
        ...
        startMenu.setPosition(baselayer.getContentSize().width/2,50);
